odoo.define('dn_base.web_editor', function (require) {
	'use strict';
	...
	var ImageDialog = Widget.extend({
		...
		init: function (parent, media, options) {
			this._super.apply(this, arguments);
			this.options = options || {};
			var acceptUploadTypes = 'application/msword, application/vnd.ms-powerpoint';
            //acceptUploadTypes +=',application/csv';
            acceptUploadTypes +=',application/pdf, text/plain';
            acceptUploadTypes +=',image/jpeg,image/jpg,image/gif,image/png';
			this.accept = this.options.accept || this.options.document ? acceptUploadTypes : "image/*";
		...
		form_submit: function (event) {
			...
			window[callback] = function (attachments, error) {
				...
				if (error) {				
					self.file_selected(attachments, error);
					return;
				}
				if (!attachments) {				
					self.file_selected(attachments ,"Invalid Attachment");
					return;
				}
	...
	var custom_web_editor = require('web_editor.widget');
	
	custom_web_editor.MediaDialog = MediaDialog; 

	return custom_web_editor;
});
